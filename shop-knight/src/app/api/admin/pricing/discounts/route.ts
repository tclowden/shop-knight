...
export async function POST(req: Request) {
  ...
  try {
    const created = await prisma.pricingDiscount.create({ data: { companyId, name: parsed.data.name, active: true } });
    return NextResponse.json(created, { status: 201 });
  } catch {
    return NextResponse.json({ error: 'Discount already exists or could not be created' }, { status: 409 });
  }
}
