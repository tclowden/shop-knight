...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  const item = await prisma.machineRate.findFirst({ where: withCompany(companyId, { id, active: true }) });
  if (!item) return NextResponse.json({ error: 'Machine rate not found' }, { status: 404 });
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const { id } = await ctx.params;
  const existing = await prisma.machineRate.findFirst({ where: withCompany(companyId, { id, active: true }) });
  if (!existing) return NextResponse.json({ error: 'Machine rate not found' }, { status: 404 });
  ...
  try {
    ...
    return NextResponse.json({ error: 'Machine rate already exists or could not be updated' }, { status: 409 });
  }
}
