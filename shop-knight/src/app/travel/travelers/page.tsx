...
export default function TravelersPage() {
  ...
  async function createTraveler(e: FormEvent) {
    ...
    const res = await fetch('/api/travel/travelers', {
      ...
      body: JSON.stringify({ fullName, email, phone, knownTravelerNumber }),
    });
    if (!res.ok) return;
    setFullName('');
    setEmail('');
  ...
  return (
    <main className="mx-auto max-w-7xl bg-[#f5f7fa] p-6 text-slate-800 md:p-8">
      ...
      <form onSubmit={createTraveler} className="mb-4 grid grid-cols-1 gap-2 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-4">
        ...
        <button className="inline-flex h-11 items-center justify-center rounded-lg bg-emerald-500 px-3 text-sm font-semibold text-white hover:bg-emerald-600 md:col-span-4">Add Traveler</button>
      </form>
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            ...
            {items.length === 0 ? (
              <tr><td className="px-4 py-8 text-center text-slate-500" colSpan={5}>No travelers yet.</td></tr>
            ) : null}
          </tbody>
        </table>
      </section>
