...
export default function NewStorageItemPage() {
  ...
  <form onSubmit={submit} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="grid grid-cols-1 gap-3 md:grid-cols-2">
    ...
    <label className="text-sm font-medium text-slate-700"><span className="mb-1 block">Bay</span><select className="field" value={rackId} onChange={(e) => setRackId(e.target.value)} required>{filteredSpaces.length === 0 ? <option value="">No bays</option> : racks.map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}</select></label>
