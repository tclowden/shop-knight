...
export default function EditLaborRatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then(async ({ id: rateId }) => {
      setId(rateId);
      const res = await fetch(`/api/admin/pricing/labor-rates/${rateId}`);
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        setError(typeof data?.error === 'string' ? data.error : 'Failed to load labor rate');
        setLoading(false);
        return;
      }
      ...
      setForm({
        ...
        machineCharge: String(data?.machineCharge ?? '0'),
  ...
  async function onSubmit(e: FormEvent) {
    ...
    const res = await fetch(`/api/admin/pricing/labor-rates/${id}`, {
      method: 'PATCH',
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      setError(typeof data?.error === 'string' ? data.error : 'Failed to update labor rate');
      setSaving(false);
      return;
    }
  ...
  return (
    <PricingPageShell title="Edit Labor Rate" description="Update this labor rate record." backHref="/admin/pricing/labor-rates" backLabel="Back to Labor Rates">
      <form onSubmit={onSubmit} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        {loading ? <p className="text-sm text-slate-500">Loading labor rate…</p> : (
          <>
            ...
            <div className="mt-4 flex items-center gap-2">
              <button type="submit" disabled={saving} className="inline-flex h-11 items-center justify-center rounded-lg bg-emerald-500 px-4 text-sm font-semibold text-white hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-60">{saving ? 'Saving…' : 'Save Labor Rate'}</button>
