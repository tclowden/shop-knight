...
export default function EditMachineRatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <PricingPageShell title="Edit Machine Rate" description="Update this machine rate record." backHref="/admin/pricing/machine-rates" backLabel="Back to Machine Rates">
        ...
        {loading ? <p className="text-sm text-slate-500">Loading machine rate…</p> : (
          <>
            ...
            <div className="mt-4 flex items-center gap-2">
              <button type="submit" disabled={saving} className="inline-flex h-11 items-center justify-center rounded-lg bg-emerald-500 px-4 text-sm font-semibold text-white hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-60">{saving ? 'Saving…' : 'Save Machine Rate'}</button>
