...
export default function ProductsAdminPage() {
  ...
  async function createProduct(e: FormEvent) {
    ...
    const res = await fetch('/api/admin/products', {
      ...
      body: JSON.stringify({ sku, name, description, salePrice, costPrice, taxable }),
    });
  ...
  return (
    <main className="mx-auto max-w-6xl p-8">
      ...
      <form onSubmit={createProduct} className="mb-4 grid grid-cols-1 gap-2 rounded border border-zinc-800 p-3 md:grid-cols-6">
        ...
        <button className="rounded bg-blue-600 px-3 py-2 md:col-span-6">Create Product</button>
      </form>
      ...
      <div className="overflow-hidden rounded border border-zinc-800">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            ...
            {products.length === 0 ? (
              <tr>
                <td className="p-3 text-zinc-400" colSpan={6}>No products yet.</td>
              </tr>
            ) : null}
          </tbody>
        </table>
